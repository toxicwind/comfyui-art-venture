...
def build_lama_model(config_p: str, ckpt_p: str, device="cuda"):
    ...
    if model_register is not None and model_register.model_path == ckpt_p:
        return (model_register.model, model_register.config)
    elif model_register is not None:
        del model_register
        torch_gc()
    ...
    if os.path.basename(os.path.dirname(predict_config.model.path)) == "models":
    ...
    model.to(device)
    model.freeze()
    model_register = ModelRegister(model, predict_config, ckpt_p)
...
def inpaint_img_with_lama(
    ...
    model, predict_config = build_lama_model(config_p, ckpt_p, device)
